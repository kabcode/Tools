...
int main(int argc, char* argv[])
{
	...
	// Add source position to projection geometry file
	std::ofstream FileStream;
	FileStream.open(GeometryFileName.string(), std::ios_base::app);
	FileStream << SourcePosition;
	FileStream.close();

	return EXIT_SUCCESS;
}

int TestProjection(std::string filename)
{

	std::cout << filename << std::endl;
	filename.append("\\Fixed3D.nrrd");
	auto reader = ImageReaderType::New();
	reader->SetFileName(filename);

	auto proj = InternalImageType::New();
	...
	size[1] = 1000;
	size[2] = 1;
	InternalImageType::RegionType reg(idx, size);
	proj->SetRegions(reg);
	proj->Allocate();

	InternalImageType::SpacingType spac;
	spac[0] = 0.5;
	spac[1] = 0.5;
	spac[2] = 1;
	proj->SetSpacing(spac);

	InternalImageType::PointType or ;
	or .Fill(0);
	proj->SetOrigin(or );
	proj->Update();

	auto detim = InternalImageType::New();
	InternalImageType::RegionType reg2(idx, size);
	detim->SetRegions(reg2);
	detim->Allocate();
	detim->SetSpacing(spac);
	detim->SetOrigin(or );

	ProjectionGeometryType::VectorType rowdir;
	rowdir[0] = 1;
	rowdir[1] = 0;
	rowdir[2] = 0;
	ProjectionGeometryType::VectorType coldir;
	coldir[0] = 0;
	coldir[1] = 1;
	coldir[2] = 0;
	ProjectionGeometryType::PointType src;
	src[0] = 0;
	src[1] = 0;
	src[2] = 800;
	ProjectionGeometryType::PointType det;
	det[0] = -(500. / 2);
	det[1] = -(500. / 2);
	det[2] = -400.;
	...
	auto Rot = EulerTransformType::New();
	Rot->SetComputeZYX(true);
	Rot->SetRotation(DegreesToRadians(90), 0, DegreesToRadians(90));
	auto R = Rot->GetMatrix();

	auto Tra = EulerTransformType::New();
	EulerTransformType::OutputVectorType Tran;
	Tran[0] = 0;
	Tran[1] = 0;
	Tran[2] = 0;
	Tra->SetTranslation(Tran);
	...
	src = Tra->TransformPoint(src);
	det = Tra->TransformPoint(det);

	std::cout << src << std::endl;
	std::cout << det << std::endl;

	auto geom = ProjectionGeometryType::New();
	geom->AddProjection(src, det, rowdir, coldir);

	auto projector = ProjectorType::New();
	projector->SetGeometry(geom);
	projector->SetInput(0, proj);
	projector->SetInput(1, reader->GetOutput());
	...
	InternalImageConstIterator ProjectorImageIt(projector->GetOutput(), projector->GetOutput()->GetLargestPossibleRegion());
	ProjectorImageIt.GoToBegin();
	InternalImageIterator DetectorImageIt(detim, detim->GetLargestPossibleRegion());
	DetectorImageIt.GoToBegin();
	...
	auto changer = ImageInformationChangeFilterType::New();
	changer->SetInput(detim);
	changer->SetOutputOrigin(det);
	changer->ChangeOriginOn();
	auto Normale = CrossProduct(rowdir, coldir);
	...
	std::cout << geom->GetSourcePosition(0) << std::endl;
	std::cout << dir << std::endl;

	auto writer = ImageWriterType::New();
	writer->SetInput(changer->GetOutput());
	writer->SetFileName("DRR.nrrd");
	...
	ExtractionFilter->SetDirectionCollapseToIdentity();
	auto inputRegion = projector->GetOutput()->GetLargestPossibleRegion();
	auto sizes = inputRegion.GetSize();
	sizes[2] = 0; // we extract along y direction
	auto start = inputRegion.GetIndex();
	start[2] = 0;
	InternalImageType::RegionType desiredRegion;
	desiredRegion.SetSize(sizes);
	desiredRegion.SetIndex(start);
	ExtractionFilter->SetExtractionRegion(desiredRegion);

	auto DRRImageWriter = OutputImageWriterType::New();
	DRRImageWriter->SetInput(ExtractionFilter->GetOutput());
	DRRImageWriter->SetFileName("DRR2.nrrd");
